...
router.post('/login', (req, res) => {
    ...
    // Verify user credentials using the LoginModel
    if (loginModel.verifyUser(username, password)) {
        ...
        // Render an error if credentials are invalid
        res.status(400).send({
            success: false,
...
router.get('/check-auth', (req, res) => {
    if (sessionManager.isAuthenticated(req)) {
        // If the user is authenticated, respond with a success message and username
        res.status(200).send({
            success: true,
